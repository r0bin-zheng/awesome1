...
@asyncio.coroutine
def create_pool(loop, **kw): #*args是非关键字参数，用于元组，**kw是关键字参数，用于字典，这里的**kw是一个dict
...
#python数据库流程: 开始->connection链接(道路)->cursor光标(货车)->cursor.execute执行事务->正常conn.commit提交 ->关闭corsor->关闭connection->结束
#                                                                                  ->异常conn.rollback回滚->
    log(sql,args)
    global __pool
    # 666 建立游标
    ...
    #紧跟with后面的语句被求值后，返回对象的__enter__()方法被调用，这个方法的返回值将被赋值给as后面的变量。当with后面的代码块全部被执行完之后，将调用前面返回对象的__exit__()方法
        ...
        #cur为conn的一个光标，以dict字典(映射)的形式展示
        yield from cur.execute(sql.replace('?', '%s'), args) #SQL语句的占位符是?，而MySQL的占位符是%s，select()函数在内部自动替换。注意要始终坚持使用带参数的SQL，而不是自己拼接SQL字符串，这样可以防止SQL注入攻击。
        if size:
...
class Field(object):
    ...
    def __str__(self): #__str__: 把一个类的实例变成str
        # 返回 表名字 字段名 和字段类型
...
class ModelMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        for k, v in attrs.items():
            if isinstance(v, Field): #isinstance()判断v是否是Field类型
